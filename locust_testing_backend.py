...
with open('backend/most_recent_items.json') as f:
    item_ids = [item['id'] for item in json.load(f)['data']['items']]

with open('backend/most_recent_tasks.json') as f:
...
class WebsiteUser(HttpUser):
    ...
    def get_tasks(self):
        ...
        random_search = ''.join(random.choices(string.ascii_lowercase, k=random.randint(0, 2)))
        random_isKappa = 'true' if random.randint(0, 1) == 0 else 'false'
        random_isLightkeeper = 'true' if random.randint(0, 1) == 0 else 'false'
